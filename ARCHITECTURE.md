...
| `apps/analytics-purge` | Hosted-only Cloudflare Cron Worker + one Container for Spark/Iceberg physical analytics deletion. D1 remains the job and retry authority; this app only replaces the external scheduler/runtime.                                                                            |
...
| `packages/shared`      | Wire format, schemas (Valibot), event types, constants — the contract everything compiles against.                                                                                                                                                                          |
...
- **Loader snippet < 2 KB** inline: starts a pre-buffer (captures `error`, `unhandledrejection`, early clicks, nav timing), fetches `/v1/config`, and makes the deterministic session sampling decision before it downloads the recorder. Sampled-out visits stop there. Sampled-in visits load the content-hashed recorder URL from config; that file is immutable for one year. The stable `/or-recorder.js` remains the safe compatibility fallback when the small config gate is unavailable. No script blocks page rendering.
...
- **Config-driven capture (dashboard-controlled)**: the loader fetches `GET /v1/config` with the public recorder key before loading rrweb. The Worker serves the D1-backed project config through persistent KV with a 30 s edge cache. Sampling, masking/block rules, policy version, and canvas capture apply on that page load with no customer deploy. The successful response is handed to the recorder in memory, so the full SDK does not repeat the request. The SDK keeps local sampling as a ceiling and ignores invalid remote CSS selectors. A timeout, server error, or malformed config may load the stable compatibility bundle, but the full recorder still fails closed to no capture; only an explicit 404 from an older Worker falls back to local settings for upgrade compatibility. Console/network toggles are stored for the planned lazy capture chunks but do not claim to collect data today. Masking rule sets are versioned (`maskPolicyVersion`) for auditability.

- **Replay assets stay out of the recorder**: `collectFonts` remains off and the browser never uploads stylesheet, font, or background-image files. This avoids extra customer-page CPU, memory, and upload bytes. Public assets are handled after finalization (§3); authenticated/private assets remain a documented fidelity limit.
...
- Headers: `x-or-key` (project recorder key), `x-or-session`, `x-or-tab` (per-tab id — part of the idempotency key), `x-or-seq` (monotonic per-tab batch counter), `x-or-flags`.
...
- **The ingest path and Session DO never decompress replay payloads.** The sidecar gives those hot paths everything they need for search, indexing, and the live timeline; gzip bytes pass through DO → R2 untouched. A separate, capped, post-finalize Queue job may inflate at most three full snapshots only when replay-asset preservation is enabled. It never runs on ingest and never rewrites the immutable recording.
...
- `sessionId` = UUIDv7 stored in `sessionStorage` + a sliding first-party cookie; survives SPA navigation and multi-tab (tab id in batch metadata). The cookie refreshes on the SDK's existing throttled touch write, not on every event. Browser rotation and server close share the same `>= 10 minutes idle` boundary. Before an idle tab rotates, it checks the still-valid shared cookie and rejoins that active session instead of overwriting it with a new id. A server `closed` response always wins and forces the next batch onto a fresh id; one session-change coordinator serializes both cases. This timeout is the session-identity boundary and crash fallback, not a product-visibility delay: a recording appears after its first accepted batch and keeps the same id while final details complete in the background.
- Deterministic **sampling decided client-side after the config request and before the recorder bundle downloads**, so unsampled sessions download no recorder and send no replay batches. Server re-checks to prevent abuse.
...
p/{projectId}/{sessionId}/asset-map.json
replay-assets/sha256/{contentHash}
```
...
- The optional asset map is private, immutable, and retention-bound with its session prefix. It is the only asset object that stores original replay URL strings; D1 stores URL hashes and authorization mappings only. Asset bytes are content-addressed and immutable. The sweeper removes unused project mappings and then unreferenced byte objects. Authenticated routes require an exact `(project, session, hash)` mapping and keep the same five-minute browser deletion bound as segments.
- Retention has two separate clocks. Recordings remain playable for **90 days by default** (projects may choose another supported recording period); the retention sweeper then deletes replay objects and D1 playback rows. Scrubbed finalized analytics remain queryable for **730 days from the session start**. Recording expiry schedules that later warehouse removal but does not publish an immediate analytics tombstone or discard a pending analytics export. If a rare long outage leaves the scrubbed analytics sidecar unsent at recording expiry, replay access is still fenced and the replay objects are removed, but that small sidecar and its D1 source row remain until export succeeds; the next sweep removes both. An explicit user, privacy, project, or account deletion still fences replay and analytics immediately, then completes the Iceberg transaction, compaction, and snapshot expiry within 24 hours. The recording bucket may transition eligible objects to Infrequent Access after 30 days. Catalog-owned R2 objects are never deleted directly. Don't rely on lifecycle rules for per-tenant policy.
...
- **Pipelines → Iceberg → R2 SQL for finalized analytics.** The ingest path never writes the warehouse. Finalization first commits a stable, scrubbed export job to a D1 outbox in the same transaction as the session and billing row. A scheduled exporter retries those jobs into one structured Pipeline stream, which fans out into `analytics_sessions`, `analytics_events`, and `analytics_deletions` Data Catalog tables. Pipelines adds `__ingest_ts` and day partitioning. Pipelines and R2 SQL cannot delete selected rows, so hosted physical erasure uses a Cloudflare Cron Worker to start one Cloudflare Container running the partition-aware Spark/Iceberg writer plus snapshot expiry; D1 still owns leases, retries, and the two-check completion proof. It never uses DuckDB or direct object deletion. Reads dedupe stable export IDs because a producer crash can resend an accepted record. A D1 watermark advances only after R2 SQL proves that every distinct export ID through that sequence is visible; stats and their linked recording list use that same `warehouse_version`. Recording expiry leaves these analytics rows live and stores a day-730 erasure job; privacy deletion makes the job due immediately. Legacy `retention_expired` tombstones are ignored by reads because they represented recording expiry, not an analytics-delete request. Pipeline or R2 SQL failure never loses replay data, never changes billing, and never becomes a fake zero dashboard. Hosted production returns cached-stale data or a clear analytics error instead of silently falling back to D1. The self-host template keeps the warehouse optional until the operator provisions the paid/beta services. Exact contract and rollback: `docs/specs/f4-r2-analytics-cutover.md`.
...
- For finalized private replays, the dashboard also requests the small asset map and fetches at most 20 MiB of mapped bytes with six concurrent requests. It sanitizes cached CSS, resolves nested dependencies to parent-created `blob:` URLs, and revokes every blob when the player is destroyed. The replay iframe keeps `connect-src 'none'`; it never contacts the recorded site or a third party. Missing, private, unsafe, oversized, or late assets fail individually and playback continues with the existing safe fallback.
...
- Public share pages do not receive replay asset maps or asset bytes in this release; the new fidelity path is authenticated-only.
...
### Workspace, Website, and journey identity

The product hierarchy is **team/account → Workspace → Websites**. A Workspace is one product, such as Noodle. It may contain related Websites such as `example.com`, `app.example.com`, and `checkout.example.com`, even when each Website is shipped from a different repository. Existing storage and route names keep `project` as the internal name for a Workspace so old recordings and APIs do not need a destructive rename.

The product presents that hierarchy before its implementation. Onboarding asks a person to add a Website, copy its installation script, and check the connection. It never asks them to create, recover, or understand a recorder key. The first Website names the new Workspace from its hostname by default; a later Website is added in the context of the current Workspace. Recorder keys remain an internal installation and revocation boundary that advanced Settings may expose to developers when needed.

Each Website has its own stable Website id, exact origin allowlist, and active recorder key. Adding the same origin again is idempotent: it returns the existing Website and pending setup instead of minting another key. The pending raw key is encrypted with `WEBSITE_KEY_WRAP_SECRET`, can be restored only while setup is unfinished, and is deleted after that Website sends its first accepted event. An origin is not proof that a person owns a domain, so the product must not reveal whether another account has registered it; domain ownership is a separate future verification flow.

Recorder-key identity answers **which Website sent this batch**. Session identity answers **which Workspace journey this visitor is continuing**. Every Website origin is checked exactly and owns its own recorder key. Related HTTPS subdomains must each be added as a Website and install that Website's snippet, but all Website keys in one Workspace receive the same `sessionScope` while the Worker derives a public-suffix-safe cookie domain for their shared first-party visitor identity. Unrelated root domains cannot share a first-party browser cookie and therefore get their own recordings. Session state and manifests keep the Website ids seen during the journey so one Workspace session can be traced back to all contributing sources.
...
| Recording R2 storage (0.5 MB × 90 d; IA after day 30)                                      | ~$0.0000175 per session         |
...
- **Queue consumer failure** → per-message try/catch (never fail the whole batch), explicit `ack`/`retry`, DLQ after max retries; the D1 session, exact usage change, and stable analytics outbox rows commit together and are idempotent. The session head remains `finalizing` and the immutable manifest stays playable until a retry commits, so a control-plane delay cannot make the recording disappear. Replay-asset work is a second Queue message with its own completion marker and retry budget; failure lowers styling fidelity but never blocks or corrupts playback.
...
1. **Server never inflates your data on the ingest path** — compressed on the user's device, stored compressed, and normally decompressed only in the viewer's browser. The default-on replay-style job is the narrow exception: after finalization it reads at most three bounded full snapshots to identify public assets, and a project owner can turn it off. Encrypted recordings always skip it. Other content-level features (search, detectors, redaction) remain explicit opt-ins processed at rest (§4 lane); the Strict tier turns inspection off contractually and the E2E tier (customer keys, wire bit 1) makes it impossible.
...
2. `packages/sdk` recorder with worker-thread compression; conformance tests against the wire format; bundle-size budget in CI (32 KB target, fail > 36 KB gzip; loader fails > 2 KB).
