...
- **Bounded seek checkpoints**: rrweb takes a full snapshot every 4 minutes or 5,000 emitted events, whichever comes first. The event-count bound protects mutation-heavy pages; the time bound protects ordinary long sessions. The SDK records each full-snapshot timestamp in the uncompressed batch sidecar so the player can seek without rebuilding the session from its first event.
...
- **Zero-egress visual sealing**: loaded `<img>` pixels are stored inside the recording with per-image pixel and byte limits, so replay never contacts the customer's site. Canvas capture is an explicit project opt-in because canvas pixels cannot be text-masked. When enabled, the SDK records capped, deduplicated WebP frames at 2 FPS; the player accepts only the fixed `clearRect` + inline-image frame shape and rejects every other canvas command. The replay iframe remains scriptless and has no network access.
...
`manifest.json` per session: segment list (key, byte size, time range, batch count, bounded checkpoint positions), the merged **index sidecar timeline** (clicks, errors, navs, rage-click detections, custom events with timestamps) — this powers the player's activity bar, skip-inactivity, jump-to-error, and nearest-checkpoint seek planning _without downloading any segment_. The DO copies checkpoint positions from the sidecar and never inflates replay payloads.
...
- Loader fetches manifest → paints the timeline instantly from the index sidecar → fetches the first needed segment → first frame typically < 1 s. Seeking starts at the nearest full-snapshot checkpoint at or before the target, cancels stale seek work, and keeps only that checkpoint window in browser memory. Encoded segment responses and the active decoded event/byte window are capped before data is committed. Older manifests without checkpoint positions remain playable from segment zero and discover checkpoints as their payloads are decoded. Decompression stays per-batch in a Worker off the player's main thread.
...
1. **Hosted (our account)** — multi-tenant. Isolation: per-org D1 database, per-project R2 key prefix, per-key KV config, quotas metered in Analytics Engine. Free tier until limits (sessions/mo + storage GB), then paid. **Dashboard auth (decided 2026-07-09): BetterAuth** — in-repo OSS auth library, D1-backed, **GitHub + Google OAuth only** (no email/password stack); `members`/org authz lands with it. Considered and rejected: anonymous no-signup capability-token workspaces — unrecoverable lockout on token loss, bespoke security-critical token code, weaker abuse gate than OAuth, and the real onboarding friction is snippet install, not sign-in; a public read-only demo workspace covers try-before-signup instead. Self-host is unaffected (Cloudflare Access, mode 2 below).
