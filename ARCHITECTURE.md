...
3. Batches flush on: **adaptive timer**, 128 KB compressed, visibility change, or `pagehide`. Default cadence 15 s; the ingest response carries a `live` flag when viewers are watching this session and the SDK tightens to 3–5 s. Flush count is the main per-session cost driver (DO requests + SQLite rows written scale with it), so we only pay for real-time cadence when someone is actually watching — competitors flush every ~5 s for everyone.
4. Transport: `fetch(..., { keepalive: true })` for unload-time batches (< 64 KB keepalive limit — final batches are kept small), regular `fetch` otherwise, `sendBeacon` as last resort. Retries with backoff; on sustained failure, cap memory and drop lowest-value events first (mousemove → scroll → mutations never).
...
- Headers: `x-or-key` (project write key), `x-or-session`, `x-or-seq` (monotonic batch counter), `x-or-flags`.
...
2. **Abuse/rate control**: Workers Rate Limiting binding per key+IP; hard reject oversized bodies (> 1 MB compressed); Turnstile-free (SDK traffic), rely on key+origin+quota.
...
- API Worker authorizes (org membership) → returns manifest + short-lived signed segment URLs; segments are streamed **through the Worker from R2** with Cache API (immutable) — repeat views of a hot session are cache hits, saving R2 Class B ops and latency (egress is $0 either way). **Cache API requires a zone**: it no-ops on `workers.dev`, so the hosted plane runs on our domain and the self-host template treats a custom domain as the recommended default (playback still works without one, just uncached).
...
| DO SQLite rows written (~90: batch inserts + deletes + alarms) | ~$0.00009                       |
...
- **Session never finalized** (DO alarm loss is near-impossible, but): nightly cron reconciles R2 prefixes without D1 rows → re-emit finalize.
...
- **Hot project, not hot session** → per-session DOs shard naturally; the only shared hot resources are KV reads (edge-cached) and the rate limiter binding (built for this). No coordinator DO on the ingest path.
