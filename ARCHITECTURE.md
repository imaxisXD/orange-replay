...
```
 ...
 Dashboard/Player ◀──authenticated streamed segments (short browser cache)────┘        ▼
   ...
   └── API Worker ── R2 SQL (finalized analytics/list) ◀─ Data Catalog        ├─▶ Pipelines: scrubbed analytics rows
                     ├─ D1 (direct recording lookup + live control)           └─▶ Webhooks / integrations
...
- Segments and manifests are **immutable in R2** (the manifest is written once at finalize; live playback reads from the DO instead). Authenticated delivery uses a short private browser cache instead of a shared immutable edge cache, so a retention or privacy deletion has a clear five-minute cache bound.
...
- **Queue consumer failure** → per-message try/catch (never fail the whole batch), explicit `ack`/`retry`, DLQ after max retries; the D1 session, exact usage change, and stable analytics outbox rows commit together and are idempotent.
