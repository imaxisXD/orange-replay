...
Recorder-key identity answers **which Website sent this batch**. Session identity answers **which Workspace journey this visitor is continuing**. All Website keys in one Workspace receive the same `sessionScope`, while the Worker derives a public-suffix-safe cookie domain for related HTTPS subdomains. This lets a journey continue from `example.com` to `app.example.com` without weakening either Website's origin boundary. Unrelated root domains cannot share a first-party browser cookie and therefore keep host-only session identity. Session state and manifests keep the Website ids seen during the journey so one Workspace session can be traced back to all contributing sources.
