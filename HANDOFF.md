...
- [ ] **T3.6 settings + install pages** — ← IN FLIGHT (Codex dispatched 2026-07-04; spec: `docs/specs/t3.6-settings-install.md`). Config editor (sampling/retention/toggles/masking/origins), read-only keys list, install snippet via SDK's real `buildLoaderSnippet`, live verify off install-status. Includes two small backend gap-closers (retentionDays in config update; GET keys). Verify: full suite + visual judge of both pages.
