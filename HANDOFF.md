...
- [ ] **T4.3 observability audit + fixes** — ← IN FLIGHT (Codex dispatched 2026-07-04; spec: `docs/specs/t4.3-observability.md`). Coverage table for every unit of work, finally-emission, base/identity fields, edge→DO→queue→consumer request-id propagation test, no stray console, observability.enabled in wrangler configs, event-size hygiene.
