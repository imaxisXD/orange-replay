...
- [ ] **F4 — Analytics dashboard** (`docs/specs/f4-analytics.md`, written 2026-07-10 after a source-checked Clarity gap review): Clarity-class overview in phases — F4.0 metric correctness (the `urlCount`-counts-batches bug at `session-recorder.ts:1311` + shared `SessionFilter`), F4.1 stats endpoint + overview page on existing D1 data, F4.2 finalize-time insights reusing the player's existing `rage.ts` detector (no SDK release), F4.3 SDK capture v2 (visitor ID, referrer/UTM, web vitals, active time, dead clicks, `identify`/`track`), F4.4 users/attribution/performance cards, F4.5 heatmaps, F4.6 smart events + funnels, F4.7 AE trends. Hard rule: every metric opens its exact session set. Overlaps F3 on dead-click definition — whichever lands second reuses the first's definition verbatim.
