...
- [x] T4.4 CI — spec `docs/specs/t4.4-ci.md`; `.github/workflows/ci.yml` with check/test/budgets/e2e/mirror/build jobs — minimal permissions, concurrency cancel, timeouts, first-party actions only, zero secrets; every job command proven locally (budget gate re-verified under CI=true — local pnpm TTY purge prompt was env noise; e2e Chromium can't launch in the codex macOS sandbox but runs in my env and on ubuntu). Packing excluded from CI (vp pack dts probe fails on missing @typescript/native-preview — pre-existing tsgo/dts issue, noted for later; build job covers the dashboard production build).
...
- [~] Production Cloudflare deployment — `docs/deployment.md` + `apps/worker/wrangler.jsonc` `env.production`; public commits keep placeholder Cloudflare resource IDs and ignored local env files hold generated production values. Created prod D1, R2, KV, queues, Worker secret, default org/project/write key, and deployed static dashboard assets through the Worker. Production config now uses Wrangler `env.production` and Cloudflare `version_metadata` instead of an `APP_VERSION` var. Secret leak audit passed for current prod token/write-key values in repo files and git history; `scripts/bootstrap-prod-project.mjs` saves generated write keys to the ignored local env file and does not print them. Live smoke passed for dashboard root, `or-recorder.js`, health API, authenticated sessions API, and ingest API. Current verification: `node scripts/mirror-template.mjs --check` passed; `vp check --no-fmt` passed with the existing 3 rrweb-fork warnings; `vp test` passed 255 tests; full `vp check` is blocked only by existing formatting issues in `docs/specs/t5.1-replay-to-repro.md` and `pnpm-workspace.yaml`. GitHub auto-deploy is not connected yet because this local repo has no GitHub remote/repo and the current Cloudflare OAuth token gets 403 from the Workers Builds API.
