...
- [ ] **T3.5 session detail: player embed** — ← IN FLIGHT (Codex dispatched 2026-07-04; spec: `docs/specs/t3.5-detail-player.md`). Player viewport + mock player bar (dashed timeline, amber playhead, kbd), event sidebar with seek, live-follow fallback. Verify: record a real session via the demo site, judge playback + bar visually vs the mock.
