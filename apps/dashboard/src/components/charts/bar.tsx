...
export interface BarProps {
  ...
  stroke?: string;
  /** 1px outline painted on the bar rect itself (unlike `stroke`, which only
   * feeds the tooltip dot). Use a darker shade of the fill to define the bar
   * edge without brightening it. */
  rectStroke?: string;
  ...
  if (animationType === "fade") {
    return (
      <motion.rect
        ...
        ry={ry}
        stroke={rectStroke}
        strokeWidth={rectStroke === undefined ? undefined : 1}
...
/** Paints a second layer (e.g. a texture pattern) over the `<Bar>` sharing its
 * `dataKey`, at identical geometry. Flagged as a depth layer so it does not
 * register as its own series — otherwise the band would split into groups. */
export function BarOverlay(props: BarProps) {
  return <Bar {...props} />;
}

BarOverlay.displayName = "BarOverlay";
(BarOverlay as { __isBarDepthLayer?: boolean }).__isBarDepthLayer = true;
