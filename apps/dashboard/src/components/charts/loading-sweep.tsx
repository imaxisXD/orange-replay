...
function SkeletonBars({
  ...
  const bandSize = (isHorizontal ? innerHeight : innerWidth) / heights.length;
  const barThickness = bandSize * barFraction;
  const bandOffset = (bandSize * (1 - barFraction)) / 2;
  const baselinePosition = isCenter
    ? (isHorizontal ? innerWidth : innerHeight) / 2
    : isHorizontal
      ? 0
      : innerHeight;
  const availableLength = isCenter
    ? (isHorizontal ? innerWidth : innerHeight) / 2
    : isHorizontal
      ? innerWidth
      : innerHeight;
  ...
  return (
    <>
      {heights.map((value, i) => {
        ...
        const barLength = Math.max(1, availableLength * (value / 100));
        const x = isHorizontal
          ? sign === 1
            ? baselinePosition
            : baselinePosition - barLength
          : i * bandSize + bandOffset;
...
export interface BarLoadingSkeletonProps {
  ...
  barCount?: number;
  /** Direction the placeholder bars grow. Default: `"vertical"` */
  orientation?: "vertical" | "horizontal";
