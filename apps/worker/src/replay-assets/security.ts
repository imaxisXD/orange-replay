...
export async function fetchPublicReplayAsset(
  ...
  const timedFetch: typeof fetch = (input, init) =>
    fetchWithTimeout(fetchFn, input, init, timeoutMs);
  const resolveHost =
    options.resolveHost ?? ((hostname) => resolvePublicHost(hostname, timedFetch));
  let url = safePublicAssetUrl(sourceUrl, options.baseUrl, options.blockedHosts);
  if (url === null) return null;

  for (let redirects = 0; redirects <= MAX_REDIRECTS; redirects += 1) {
    if (!(await hostnameResolvesPublicly(url.hostname, resolveHost))) return null;
    ...
    const bytes = await readBodyCapped(response, byteLimit);
...
async function resolvePublicHost(hostname: string, fetchFn: typeof fetch): Promise<string[]> {
  const results = await Promise.all(
    (["A", "AAAA"] as const).map((type) => resolveDnsType(hostname, type, fetchFn)),
  );
...
async function resolveDnsType(
  ...
  const bytes = await readBodyCapped(response, MAX_DNS_RESPONSE_BYTES);
