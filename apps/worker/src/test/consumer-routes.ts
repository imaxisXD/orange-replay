...
async function failEventInsert(url: URL, env: Env): Promise<Response> {
  ...
  await env.IDX_00.exec("DROP TRIGGER IF EXISTS test_fail_event_insert");
  await env.IDX_00.exec(
...
async function failSessionDelete(url: URL, env: Env): Promise<Response> {
  ...
  await env.IDX_00.exec("DROP TRIGGER IF EXISTS test_fail_session_delete");
  await env.IDX_00.exec(
