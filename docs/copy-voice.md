...
Beat one is proof: real data from our own site. No adjective can fake that claim.
...
**Never use an em dash (Unicode U+2014). No exceptions.** Split the thought
into two sentences, or use a comma or colon.
...
5. **Second person, present tense.** Talk to the reader about their
   ...
   only when accuracy demands it. The hedge then builds trust.
...
- Never use an em dash. Use a period, comma, or colon. No exceptions.
