...
- **Status pills** (`src/components/status-pill.tsx` — single source of truth): full-radius, 11px medium, 6px dot. `ok` (border-border, muted text, green dot) · `err` (`#ffb3b0` on `rgba(244,83,78,0.07)`, red dot) · `rage` (`#ffd9a0` on `rgba(245,166,35,0.07)`, amber dot) · `neutral` chip (no dot). Copy: "clean", "1 error"/"N errors" (pluralize), "N rage" (never pluralized).
...
- **Activity sparklines** (sessions table Activity column, needs per-session buckets): 84×16px, 1.5px-gap bars, `#26262d` with red error bars.
