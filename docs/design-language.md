...
- **Tier-2 tabs** (same bg, `px-[28px] border-b`): 13px, `text-muted-foreground`, `py-[10px] px-[13px]`, `border-b-2 -mb-px`; active = `text-foreground border-amber font-medium`. Active tab icon sits on an **app-icon plate** (the local tab lab's C2 teal colorway): 17px `rounded-[5px]` square, deep teal gradient fill (`color-mix(in oklab)` 92%-teal/black → 74%-teal/black, 160deg), 1px inset rim one step lighter than the fill (`color-mix` 70% teal/white), dark glyph (`text-background`, 12px, stroke 1.75), soft teal bloom (`0 0 8px -1px` `color-mix` 70% teal/transparent); inactive tabs keep the same 17px transparent box so labels never shift. Live tab glyph = `LiveStreaming02`. Only real routes get tabs.
...
- **Stat strip** (see mock + session-detail): `.lit` flex row; cells `flex-1 px-[18px] py-[15px] border-r border-dashed border-dash`; label 11.5px muted; value 21px Uncut Plan8 regular with normal tracking, **amber when it's a nonzero warning count** (errors, rages).
- **Overview metric cards**: the eight focal values alone use 24px Departure Mono regular at 1.1 line height with normal tracking. Percentage symbols inherit the same face and treatment as their number. Never synthesize a heavier Departure Mono weight.
