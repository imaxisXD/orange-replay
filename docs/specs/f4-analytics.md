...
- **The Analytics Engine write is useless for dashboards as-is**: one datapoint per ingest batch, blobs=[country], doubles=[1, bytes] (`apps/worker/src/ingest/handler.ts:602-613`); binding declared but unprovisioned. AE is deferred to F4.7 with a per-session finalize datapoint. AE is sampled + 3-month retention: trends only, never the source for exact counts or session sets — D1 is.
...
- **Quick backs are derivable for SPA-internal navigation only** (nav index events don't fire on full page loads or external exits). Ship as "quick backs (in-app)".
...
Scope: `apps/worker/src/do/**`, `apps/worker/src/api/**`, `packages/shared/**`.

1. **Fix `urlCount`.** Add `lastUrl?: string` to `SessionState`; in `updateStateWithBatch`, increment `urlCount` only when `clampedIndex.u` differs from `lastUrl`, then update `lastUrl`. Default via `normalizeSessionState` for in-flight persisted states (miscount of at most one on the deploy boundary is acceptable). Unit-test: same-URL batches count once; A→B→A counts 3.
   - **Historical skew**: `url_count` rows written before this fix are inflated. The stats endpoint must expose the deploy watermark (a `pages_since` timestamp constant) and the dashboard shows pages/session only for date ranges at/after it.
...
3. **Shared filter object.** One zod schema in `packages/shared` — `SessionFilter`: `{ from?, to?, country?, region?, device?, browser?, os?, entry_url_prefix?, has_errors?, has_rage?, min_duration_ms? }` — used by BOTH the sessions list and the F4.1 stats endpoint. Plain query params, no opaque token (saved segments get IDs later, out of scope). Every filter key maps to an indexed/prepared D1 predicate; prepared statements only.

DoD: gates green; sessions endpoint accepts the full filter; a fixture proves stats-filter ≡ sessions-filter set equality on seeded data.
...
**API**: `GET /api/v1/projects/:id/stats?<SessionFilter>` (authz middleware as all routes) returning, in one response: session count; avg + p50 duration; total clicks; pages/session (post-watermark only, see F4.0); breakdowns (top-N + share) by country/region, device, browser, os, entry page; error groups (`session_events` kind=`error` GROUP BY detail, count + affected sessions); live-now count (presence shards). Every breakdown row includes the `SessionFilter` delta that selects it. Cache API 60s per (project, canonical filter string). Wide event per request (route, cache_hit, duration_ms). D1 stays the exact source; verify each query with EXPLAIN QUERY PLAN against the `(project_id, started_at DESC)` index and add covering indexes only where the plan demands.
...
## F4.2 — Finalize-time derived insights (no SDK release)

Scope: `packages/shared/**` (or `packages/player` extraction), `apps/worker/src/do/**`, `apps/worker/src/consumer/**`, migration `000N_insights.sql`, dashboard Insights card.

All derivation runs at finalize over the **sidecar timeline only** (never payloads), inside the existing finalize budget; pure functions, unit-fixtured.

1. **Rage clicks**: move `detectRageClickBursts` to a shared package (player re-imports; behavior byte-identical, existing tests keep passing). Run at finalize over click index events (normalized coords × viewport → px); store burst count in the existing `rages` column. The always-zero column and the dashboard's existing rage badges become real.
2. **`max_scroll_depth`** (0–100 int): max over scroll index events.
3. **`quick_backs`** (int): nav sequence A→B→A where dwell on B < 10s (constant, tunable). In-app only; label accordingly.
...
5. New columns via migration (single-line SQL — D1 `exec()` splits on newlines); queue consumer writes them idempotently; stats endpoint gains an `insights` block (rage %, quick-back %, avg interaction time, avg max-scroll) with session-set doorways (`has_rage=1` etc.); dashboard Insights card in the Clarity slot.
...
Scope: `packages/sdk/**`, `packages/shared/**`, ingest/DO/consumer plumbing, migration, e2e. Bundle budget holds: core ≤ 20 KB gz (measure; web-vitals observers are hand-rolled `PerformanceObserver` code, no new dependency without justification).
...
2. **Attribution**: at session start capture `document.referrer` **host only** + allowlisted UTM keys (`utm_source/medium/campaign/term/content`) from the entry URL before scrubbing strips them. Columns: `referrer_host`, `utm_source`, `utm_medium`, `utm_campaign`.
...
4. **Real active time + page lifecycle**: visible/hidden tracking (`visibilitychange` + interaction-refreshed heartbeat) → `active_time_ms`; explicit page-view events for MPA + SPA (fixes pages/session fully and gives per-page dimensions for F4.5/F4.6). When this lands, the dashboard swaps "Interaction time" for real "Active time"; the F4.2 estimate remains a fallback for old sessions.
...
6. **Public API**: `identify(id, traits?)` (opt-in user identity, scrubbed traits), `track(name, props?)` (alias of existing custom events), `setTag(k, v)`. Documented in the install page snippet.
...
Selector-based aggregation as the primary model (we already capture selector + normalized coords + viewport, `packages/sdk/src/scrub.ts`), coordinate bins as fallback; grouped by scrubbed URL + device class + viewport bucket. Click maps and scroll-depth maps first; attention maps need F4.3 dwell data. Aggregation tables added HERE, not before. Rendering rides the existing player/snapshot infrastructure.
...
Replace the per-ingest-batch `writeTrend` with **one datapoint per finalized session** (blobs: country, device, browser, os, entry-page; doubles: 1, duration, errors, rages, bytes). Powers trend sparklines/time-series on the overview. Sampled + 90-day retention ⇒ approximate trends only; every exact number and every doorway stays on D1.

## Non-goals (this campaign)

Bot traffic reporting (belongs to CDN/edge logs, not SDK signals — revisit with the edge-injection moat feature); excessive-scrolling metric (needs baselines); attention heatmaps before F4.3; saved segments/watchlist UI (filter params now, IDs later); a generic analytics chatbot (the AI feature stays F1's replay-to-repro direction).

## Constraints & DoD (campaign-wide)

PLAN.md ground rules apply verbatim: cost invariants are correctness (zero payload decompression on ingest/finalize, hibernation eligibility, minimal alarms, idempotent consumer writes), wide-event logging on every new route/unit of work, prepared statements only, authz on every route, registry-only UI judged against `design-final.html`, `vp check` + `vp test` green per phase. Every phase updates HANDOFF.md in the same commit.
