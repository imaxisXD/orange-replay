...
Emergency rollback must not depend on the normal release pipeline. After its acceptance gate, every normal deploy uploads the same reviewed code, assets, and versioned secrets as an inactive D1 version with a unique fallback tag before it deploys the selected backend. `deploy:prod:rollback` lists recent versions by the production Worker name, chooses the newest valid D1 fallback tag, and deploys that exact version ID at 100%. It does not read current application source, assets, generated config, migrations, or secret files. If no tag is available, it stops without changing traffic. `deploy:prod:rollback:rebuild` remains an explicit second choice that uses current local source and assets and may be blocked by them.

Normal cutover must not change a live Worker secret before the acceptance gate. Compare and R2 SQL both run the full gate. It ignores alternate local reader tokens and queries with the exact `ORANGE_REPLAY_PROD_R2_SQL_TOKEN` value intended for the new Worker. After it passes, Wrangler uploads an inactive tagged D1 fallback and then the selected version through private temporary `--secrets-file` values with `--strict --keep-vars`. Cloudflare-hosted compare and R2 builds use the same protected token for the gate and both version uploads. A failed gate therefore leaves the serving version and its live secrets unchanged.
