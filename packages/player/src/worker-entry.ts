...
const MAX_DECODED_BATCH_EVENTS = 25000;
...
const MAX_REPLAY_EVENT_ARRAY_ITEMS = 10000;
const MAX_DECODED_BATCH_SHAPE_NODES = 250000;
...
function validateFullSnapshotData(data) {
...
function validateSnapshotNode(root, errorMessage) {
...
function validateTextLikeNode(node, errorMessage) {
  if (
...
function queueSnapshotChildren(node, stack, depth, errorMessage) {
...
function validateDocumentTypeNode(node, errorMessage) {
  if (
...
function validateElementNode(node, errorMessage) {
...
function validateSnapshotAttributes(value, errorMessage) {
...
function isValidSnapshotTagName(value) {
  return (
    typeof value === "string" &&
    value.length > 0 &&
...
function validateIncrementalData(data) {
...
function validateMutationData(data) {
...
function validateArrayField(data, field) {
...
function validateRecordSafeId(value, field) {
  if (!isPlainRecord(value)) {
    throw new Error("Replay batch contains invalid mutation data.");
  }
...
function validateOptionalSafeId(value) {
  if (value !== undefined && !isSafeReplayId(value)) {
    throw new Error("Replay batch contains invalid mutation data.");
  }
}

function isSafeReplayId(value) {
  return Number.isSafeInteger(value) && Number(value) >= 0;
}

function countBoundedJsonShape(root, remainingNodes) {
