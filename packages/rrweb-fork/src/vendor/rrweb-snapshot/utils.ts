 ...
 */
import type { idNodeMap, MaskInputFn, MaskInputOptions, nodeMetaMap } from "./types";
...
export function isNodeMetaEqual(a: serializedNode, b: serializedNode): boolean {
  ...
  if (a.type === NodeType.Document) return a.compatMode === (b as documentNode).compatMode;
    ...
    return (
      ...
      JSON.stringify(a.attributes) === JSON.stringify((b as elementNode).attributes) &&
...
export function splitCssText(
  ...
  if (childNodes.length > 1 && cssText && typeof cssText === "string") {
    ...
    for (let i = 1; i < childNodes.length; i++) {
      if (childNodes[i].textContent && typeof childNodes[i].textContent === "string") {
