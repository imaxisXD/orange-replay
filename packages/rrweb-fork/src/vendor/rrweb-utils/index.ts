...
const testableAccessors = {
  Node: ["childNodes", "parentNode", "parentElement", "textContent", "ownerDocument"] as const,
...
const untaintedBaseIframeCleanup: Partial<Record<keyof BasePrototypeCache, () => void>> = {};
...
export function getUntaintedPrototype<T extends keyof BasePrototypeCache>(
  ...
  if (untaintedBasePrototype[key]) return untaintedBasePrototype[key] as BasePrototypeCache[T];
  ...
  try {
    ...
    const untaintedObject = (win as any)[key].prototype as BasePrototypeCache[T];
...
>(key: K, instance: BasePrototypeCache[K], accessor: T): BasePrototypeCache[K][T] {
  const cacheKey = `${key}.${String(accessor)}`;
  if (untaintedAccessorCache[cacheKey])
    return untaintedAccessorCache[cacheKey].call(instance) as BasePrototypeCache[K][T];
...
export function getUntaintedMethod<
  K extends keyof BasePrototypeCache,
  T extends keyof BasePrototypeCache[K],
>(key: K, instance: BasePrototypeCache[K], method: T): BasePrototypeCache[K][T] {
  const cacheKey = `${key}.${String(method)}`;
  if (untaintedMethodCache[cacheKey])
    return untaintedMethodCache[cacheKey].bind(instance) as BasePrototypeCache[K][T];
...
export function querySelectorAll(n: Element, selectors: string): NodeListOf<Element> {
