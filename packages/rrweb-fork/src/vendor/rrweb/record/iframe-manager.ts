...
import { genId } from "../../rrweb-snapshot/index.ts";
import type { CrossOriginIframeMessageEvent } from "../types";
import CrossOriginIframeMirror from "./cross-origin-iframe-mirror";
import { EventType, NodeType, IncrementalSource } from "../../rrweb-types/index.ts";
import type {
  eventWithTime,
  eventWithoutTime,
...
export class IframeManager {
  ...
  private crossOriginIframeMap: WeakMap<MessageEventSource, HTMLIFrameElement> = new WeakMap();
  ...
  public crossOriginIframeRootIdMap: WeakMap<HTMLIFrameElement, number> = new WeakMap();
  ...
  public attachIframe(iframeEl: HTMLIFrameElement, childSn: serializedNodeWithId) {
    this.mutationCb({
      adds: [
        {
    ...
    if (this.recordCrossOriginIframes)
      iframeEl.contentWindow?.addEventListener("message", this.handleMessage.bind(this));
  ...
  private replace<T extends Record<string, unknown>>(
    ...
    for (const key of keys) {
      ...
      if (Array.isArray(obj[key])) {
        obj[key] = iframeMirror.getIds(iframeEl, obj[key] as number[]) as T[keyof T];
      } else {
  ...
  private replaceIdOnNode(node: serializedNodeWithId, iframeEl: HTMLIFrameElement) {
