...
type GroupingCSSRule = CSSGroupingRule | CSSMediaRule | CSSSupportsRule | CSSConditionRule;
...
function getNestedCSSRulePositions(rule: CSSRule): number[] {
  ...
  function recurse(childRule: CSSRule, pos: number[]) {
    if (
      ...
      const rules = Array.from((childRule.parentRule as GroupingCSSRule).cssRules);
...
function initStyleSheetObserver(
  ...
  win.CSSStyleSheet.prototype.removeRule = function (this: CSSStyleSheet, index: number) {
...
function initStyleDeclarationObserver(
  ...
  win.CSSStyleDeclaration.prototype.removeProperty = new Proxy(removeProperty, {
    apply: callbackWrapper(
      (target: typeof removeProperty, thisArg: CSSStyleDeclaration, argumentsList: [string]) => {
  ...
  const handler = callbackWrapper((type: MediaInteractions) =>
    throttle(
      callbackWrapper((event: Event) => {
        ...
        const { currentTime, volume, muted, playbackRate, loop } = target as HTMLMediaElement;
        mediaInteractionCb({
          type,
          id: mirror.getId(target as Node),
          currentTime,
...
function initFontObserver({ fontCb, doc }: observerParam): listenerHandler {
  ...
  const restoreHandler = patch(doc.fonts, "add", function (original: (font: FontFace) => void) {
...
function initCustomElementObserver({ doc, customElementCb }: observerParam): listenerHandler {
  const win = doc.defaultView as IWindow;
...
type CSSGroupingProp = "CSSGroupingRule" | "CSSMediaRule" | "CSSSupportsRule" | "CSSConditionRule";
