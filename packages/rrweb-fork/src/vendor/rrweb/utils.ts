...
export function closestElementOfNode(node: Node | null): HTMLElement | null {
  ...
  const el: HTMLElement | null =
    node.nodeType === node.ELEMENT_NODE ? (node as HTMLElement) : dom.parentElement(node);
  return el;
}
...
export function isIgnored(n: Node, mirror: Mirror, slimDOMOptions: SlimDOMOptions): boolean {
...
export function getBaseDimension(node: Node, rootIframe: Node): DocumentDimension {
 ...
 */
export function getNestedRule(rules: CSSRuleList, position: number[]): CSSGroupingRule | null {
