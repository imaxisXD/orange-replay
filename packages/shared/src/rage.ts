...
// The detector compares nearby clicks pairwise. This caps the worst case at four million checks.
export const MAX_RAGE_DETECTION_CLICKS = 2_000;
...
export function detectRageClickBursts(
  ...
  const guardedClicks =
    clicks.length > MAX_RAGE_DETECTION_CLICKS ? clicks.slice(0, MAX_RAGE_DETECTION_CLICKS) : clicks;
  const sortedClicks = [...guardedClicks].sort((left, right) => left.timeMs - right.timeMs);
