...
describe("mirror-template", () => {
  it("generates the self-host template and detects drift", async () => {
    ...
    try {
      const generated = await runMirror(["--out", outDir, "--stamp", stamp]);
      ...
      const cleanCheck = await runMirror(["--out", outDir, "--stamp", stamp, "--check"]);
      expect(cleanCheck.code).toBe(0);

      await writeFile(join(outDir, "README.md"), "tampered\n", "utf8");
      const tamperedCheck = await runMirror(["--out", outDir, "--stamp", stamp, "--check"]);
