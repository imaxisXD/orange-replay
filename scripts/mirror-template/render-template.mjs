...
import { defaultTemplateDir, workerDir } from "./paths.mjs";
...
function appendAssets(lines, assets) {
  ...
  appendObject(lines, "assets", {
    ...assets,
    directory: templatePathFromWorker(directory),
  });
}

function templatePathFromWorker(pathFromWorker) {
  if (posix.isAbsolute(pathFromWorker)) {
    return pathFromWorker;
  }

  const repoPath = posix.normalize(posix.join(workerDir, pathFromWorker));
  return posix.relative(defaultTemplateDir, repoPath);
}
...
function appendObject(lines, key, value) {
  ...
  for (const [entryKey, entryValue] of Object.entries(value)) {
    pushJsonProperty(lines, entryKey, entryValue, 2);
  }
  lines.push("  },");
}
...
function formatJsonValue(value, indentLevel) {
  ...
  if (Array.isArray(value) && lines.length > 2) {
